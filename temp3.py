...
# }
#
# with h5py.File("LSTM-Actions-Recognition-main/lstm-model.h5", 'r') as f:
...
#     recipient_email = 'shridharmareppagol@gmail.com'
#
...
#                                             send_email("SPOOFING", f"Done By {name}", img_display)
#
...
#                         confidence = conf[pos]
#
#                         # Send email with the frame
#                         send_email("Suspicious",
#                                    f"Someone brought {class_name}",
#                                    frame)
#                         playsound("notification.mp3")
#
#
...
# @app.route('//run_noter_script')
# def play():
#     print("Main Template called")
#     return  render_template("summarize1.html")
# @app.route('/gp/index1')
# def index():
...
# Global variables for all streams
output_frame1 = None  # Face recognition and fake detection
output_frame2 = None  # Pose detection
output_frame3 = None  # Gaze detection
output_frame4 = None  # Weapon detection
lock1 = threading.Lock()
lock2 = threading.Lock()
lock3 = threading.Lock()
lock4 = threading.Lock()
...
# Load YOLO model for weapon detection
weapon_model_path = "runs/detect/train15/weights/best.pt"
weapon_yolo_model = YOLO(weapon_model_path)

# Load YOLO model for real/fake face detection
face_model_path = "../models/best.pt"
face_yolo_model = YOLO(face_model_path)
face_classNames = ["fake", "real"]
confidence_threshold = 0.6
...
# Load LSTM model for action recognition
with h5py.File("LSTM-Actions-Recognition-main/lstm-model.h5", 'r') as f:
    model_config = f.attrs.get('model_config')
    model_config = json.loads(model_config)
...
# Open the webcam for all streams
...
concentration_lists = []
current_concentration = 0
concentration_values = [0] * 7

# Load dlib models for gaze detection
face_detector = dlib.get_frontal_face_detector()
landmark_predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat")

# Detection statistics for fake detection
detection_stats = {
...
def generate_concentration_values(stability, focus, blink_rate):
    values = [
        stability * 0.8 + focus * 0.2,
        focus * 0.8 + stability * 0.2,
        (stability + focus + (1 - blink_rate)) / 3,
        stability * 0.6 + focus * 0.3 + (1 - blink_rate) * 0.1,
        focus * 0.6 + stability * 0.3 + (1 - blink_rate) * 0.1,
        (stability + focus) * 0.45 + (1 - blink_rate) * 0.1,
        np.mean([stability, focus, 1 - blink_rate])
    ]
    return [max(0, min(1, v)) for v in values]
...
def stream1():
    """Stream 1: Face recognition with fake detection"""
    print("Face Recognition with Fake Detection Called")
    global output_frame1, lock1, student_id, concentration_values, detection_stats
    last_attendance_check = time.time()
    attendance_times = {}
    ...
    while True:
        ...
        encode_faces = face_recognition.face_encodings(imgS_rgb, face_locations)

        # Process YOLO results for real/fake classification
        results = face_yolo_model(img, stream=True, verbose=False)

        # Default values
        name = "Unknown"
        student_id = None
        is_fake_detected = False
        fake_face_location = None

        # First, check for fake faces using YOLO
        for r in results:
            boxes = r.boxes
            for box in boxes:
                x1, y1, x2, y2 = (box.xyxy[0].cpu().numpy()).astype(int)
                conf = math.ceil((box.conf[0] * 100)) / 100
                cls = int(box.cls[0])
                class_name = face_classNames[cls]
                ...
                if conf > confidence_threshold:
                    ...
                    if class_name == "fake":
                        ...
                        fake_face_location = (x1, y1, x2, y2)

                        # Draw bounding box for fake face
                        cv2.rectangle(img_display, (x1, y1), (x2, y2), (0, 0, 255), 2)
                        cv2.rectangle(img_display, (x1, y1 - 35), (x2, y1), (0, 0, 255), cv2.FILLED)
                        cv2.putText(img_display, f"FAKE ({conf:.2f})", (x1 + 6, y1 - 10),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

                        # Send email alert for fake face
                        send_email("SPOOFING DETECTED", "A fake face/spoofing attempt detected!", img_display)
                        playsound("notification.mp3")

                    elif class_name == "real":
                        detection_stats["real_faces"] += 1
                        # Draw bounding box for real face
                        cv2.rectangle(img_display, (x1, y1), (x2, y2), (0, 255, 0), 2)
                        cv2.rectangle(img_display, (x1, y1 - 35), (x2, y1), (0, 255, 0), cv2.FILLED)
                        cv2.putText(img_display, f"REAL ({conf:.2f})", (x1 + 6, y1 - 10),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

        # Only proceed with face recognition if no fake is detected or we want to recognize real faces
        if encode_faces and not is_fake_detected:
            for encodeFace, faceLoc in zip(encode_faces, face_locations):
                matches = face_recognition.compare_faces(encodeListKnown, encodeFace)
                faceDis = face_recognition.face_distance(encodeListKnown, encodeFace)
                ...
                if len(faceDis) > 0:
                    ...
                    if matches[matchIndex]:
                        ...
                        if studentInfo:
                            ...
                            # Draw rectangle and name for recognized face
        ...
        # Display face count and detection stats
        cv2.putText(img_display,
                    f"Faces: {len(face_locations)} | Real: {detection_stats['real_faces']} | Fake: {detection_stats['fake_faces']}",
...
def stream2():
    """Stream 2: Pose detection for action recognition"""
    print("Stream 2 Called ...")
    global output_frame2, lock2, lm_list
    ...
    while True:
        ...
        if results.pose_landmarks:
            ...
            if len(lm_list) == 20:
                t1 = threading.Thread(target=detect_action, args=(model, lm_list))
                t1.start()
                lm_list = []
            ...
            cv2.rectangle(frame, (min(x_coordinate), max(y_coordinate)),
                          (max(x_coordinate), min(y_coordinate) - 25), (0, 255, 0), 1)
            frame = draw_landmark_on_image(mpDraw, results, frame)
            frame = draw_class_on_image(label, frame)
...
def stream3():
    """Stream 3: Gaze detection for concentration monitoring"""
    global output_frame3, lock3, current_concentration
    previous_gaze_vector = None
    print("Starting concentration monitoring...")
            ...
            if result.boxes is not None:
                ...
                for pos, detection in enumerate(detections):
                    if conf[pos] >= 0.7:
                        ...
                        # Send email alert
                        send_email("SUSPICIOUS ITEM DETECTED",
                                   f"Someone brought {class_name} with confidence {confidence:.2f}",
                                   frame)
                        playsound("notification.mp3")
...
# Video stream generation functions
def generate_video1():
    global output_frame1, lock1
    while True:
        with lock1:
...
if __name__ == '__main__':
    ...
    t1 = threading.Thread(target=stream1, daemon=True)
    t2 = threading.Thread(target=stream2, daemon=True)
    t3 = threading.Thread(target=stream3, daemon=True)
    t4 = threading.Thread(target=stream4, daemon=True)

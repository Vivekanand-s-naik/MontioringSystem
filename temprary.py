...
def stream_gaze_detection():
    ...
    colors = {
        'high_concentration': (0, 255, 0),  # Green
        'medium_concentration': (0, 255, 255),  # Yellow
        'low_concentration': (0, 0, 255)  # Red
    }
    ...
    while True:
        ...
        if eye_landmarks and current_gaze_vector is not None:
            ...
            if concentration > 0.8:
                concentration_color = colors['high_concentration']
            elif concentration > 0.5:
                concentration_color = colors['medium_concentration']
            else:
                concentration_color = colors['low_concentration']

            # Eye focus visualization
            if len(eye_landmarks) > 0:
                # Draw eye points
                for landmark in eye_landmarks:
                    cv2.circle(display_frame,
                               (int(landmark[0]), int(landmark[1])),
                               3, (255, 0, 255), -1)  # Magenta points

                # Gaze direction indicator
                center_x, center_y = np.mean(eye_landmarks, axis=0).astype(int)
                gaze_endpoint_x = int(center_x + current_gaze_vector[0] * 50)
                gaze_endpoint_y = int(center_y + current_gaze_vector[1] * 50)
                cv2.arrowedLine(display_frame,
                                (center_x, center_y),
                                (gaze_endpoint_x, gaze_endpoint_y),
                                (255, 165, 0), 2)  # Orange arrow showing gaze direction

            # Concentration level visualization
            concentration_bar_width = 20
            concentration_bar_height = int(display_frame.shape[0] * concentration)
            cv2.rectangle(display_frame,
                          (display_frame.shape[1] - concentration_bar_width,
                           display_frame.shape[0] - concentration_bar_height),
            ...
            cv2.putText(display_frame, f"Focus Duration: {focus_duration:.1f}s",
                        (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, concentration_color, 2)
            cv2.putText(display_frame, f"Blink Rate: {blink_rate:.2f}/s",
                        (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, concentration_color, 2)
            cv2.putText(display_frame, f"Concentration: {concentration:.2f}",
                        (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, concentration_color, 2)

            # Concentration level emoji/face marker
            if concentration > 0.8:
                emoji = "😄"  # High concentration, happy face
            elif concentration > 0.5:
                emoji = "😐"  # Medium concentration, neutral face
            else:
                emoji = "😴"  # Low concentration, sleepy face

            cv2.putText(display_frame, emoji,
                        (display_frame.shape[1] - 50, 50),
...
class TextSummarizer:
    ...
    def extract_key_points(self, text, max_words=50):
        ...
        # Take top words or truncate to max length
    ...
    def summarize(self, text):
        ...
        try:
            # Generate summary by extracting key points
            summary = self.extract_key_points(text)
            return summary
        except Exception as e:
...
class SpeechRecognizer:
    ...
    def process_chunk(self, data):
        """Process audio chunk and recognize speech"""
...
def stop_recording():
    ...
    final_text = speech_recognizer.get_final_result()
    summary = summarizer.summarize(final_text)
    return jsonify({
        "text": final_text,
        "summary": summary
    })
...
def stream_text():
    """Server-Sent Events for live text streaming"""
    def generate():
        while speech_recognizer.is_recording:
            try:
                text = speech_recognizer.text_queue.get(timeout=0.5)
                yield f"data: {text}\n\n"
            except queue.Empty:
                pass
...
def process_audio_chunk():
    ...
    try:
        audio_chunk = request.files['audio'].read()
        recognized_text = speech_recognizer.process_chunk(audio_chunk)
